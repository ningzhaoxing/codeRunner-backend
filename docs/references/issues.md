...
### 15. Worker 节点未暴露 /metrics

**文件**：`internal/interfaces/adapter/initialize/app.go:71-103`

`metrics.go` 定义了 9 个指标，但其中 5 个属于 worker 进程：`code_execution_total / code_execution_duration_seconds / container_pool_idle_count / container_pool_acquire_duration_seconds / container_pool_replenish_total`。`RunClient()` 只起了 gRPC + WebSocket，没起 HTTP server，所以 worker 没有 `/metrics`，Prometheus 抓不到这些指标，Grafana 面板永远 No data。

**修法**：worker 启动时额外开一个 HTTP server（独立端口，例如 `7980`）暴露 `/metrics`，Prometheus 静态配置中加上 worker target。
