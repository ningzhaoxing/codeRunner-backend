...
- **AI Agent 集成** — 代码学习助手，自动把博客里不完整的代码片段补全成可运行程序，解释报错、生成测试，并可经用户确认后真实执行（可选）
...
博客里的代码块绝大多数是**函数级示例**，缺少 package 声明、import、main 函数包装，读者看完想亲手跑一遍却无从下手；遇到不理解的语法或报错，只能离开页面去搜索，学习链路被打断。

Code Learning Agent 补齐这一缺口：

- **代码补全至可运行** — Agent 读取整篇文章（正文 + 所有代码块）后，自动推断并补齐缺失的脚手架（package / import / main / 依赖类型），说明补了什么，经用户确认后直接在沙箱执行
- **文章内闭环** — 读者不用跳出页面，提问、补全、运行、追问全部在对话中完成
- **基于执行结果继续推理** — Agent 拿到真实 stdout/stderr 后可以继续分析，而不是给出"看起来应该能跑"的猜测
...
详细 API 参见 [docs/agent-api.md](docs/agent-api.md)，需求与设计文档参见 [docs/context/](docs/context/)。
...
| 实现计划 | `docs/context/implementation-plans/` | 设计落地后产出的逐步实现清单 |
| 架构决策 | `docs/context/decisions/` | ADR（按需补充） |
| Issue 追踪 | `docs/references/issues.md` | 已知问题与改进项 |

> 历史上的 `docs/superpowers/{specs,plans}/` 已废弃，所有产物统一收敛到 `docs/context/`。
